...
myftw(char *pathname, Myfunc *func)
{
	...
	maxdepth = 0;
	chdir(pathname); // восстанавливать не будем
...
dopath(Myfunc* func, int depth)
{
	...
	*/
	...
	fullpath[n++] = '/';
	fullpath[n] = 0;
	if ((dp = opendir(".")) == NULL) // !!!!!!! макс. глубина рекурсии ограничена макс. кол-вом открытых файлов-каталогов на процесс
	...
	while ((dirp = readdir(dp)) != NULL) {
		...
		chdir("..");
		if ((dp = opendir(".")) == NULL)
			err_sys("невозможно открыть каталог на глубине %d",depth);
		seekdir(dp,dirpos);
	}
	//printf("%d-%ld ",depth,-telldir(dp));
